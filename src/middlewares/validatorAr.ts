...
export const validateuser = [
  check('userName')
    .notEmpty()
    .withMessage('يجب ان تسجيل الحساب ')
  ...
  check('password').notEmpty().withMessage('يجب تسجيل الرقم السري'),
  check('email').notEmpty().isEmail().withMessage('ايميل خاطى'),
...
export const validateCreateProduct = [
  check('title')
    ...
    .withMessage('يجب يكون العنوان فوق الثلاث الحروف')
    .custom(async (value: string) => {
  ...
  check('price').notEmpty().withMessage('خانه السعر مطلوبه'),
  check('description').notEmpty().withMessage('خانه الوصف مطلوبه '),
  check('categoryId').notEmpty().withMessage('خانه التصنيف مطلوبه'),
  check('quantity').notEmpty().withMessage('خانه الاعداد مطلوبه'),
]
